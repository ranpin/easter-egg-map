"""
小红书彩蛋内容采集器
使用 Playwright 模拟浏览器访问小红书搜索页，提取笔记数据。
...
async def extract_note_cards(page: Page) -> list[dict]:
    """从搜索结果页提取笔记卡片数据"""
    cards = []
    # 小红书搜索结果的选择器（可能随版本变化需调整）
    selectors = [
        "section.note-item",
        "div[class*='note-item']",
        "a[class*='cover']",
    ...
    for el in elements[:20]:  # 限制单次最多20条
        try:
            card_data = await el.evaluate("""
                (el) => {
                    const titleEl = el.querySelector('[class*="title"], .note-title, h3');
                    const authorEl = el.querySelector('[class*="author"], .user-name');
...
async def scrape_xiaohongshu(keyword: str, max_pages: int = MAX_PAGES_PER_KEYWORD) -> list[ScrapedEgg]:
    """采集指定关键词的小红书笔记"""
    results = []
    search_url = f"https://www.xiaohongshu.com/search_result?keyword={keyword}&type=general"
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for page_num in range(max_pages):
                ...
                cards = await extract_note_cards(page)
                print(f"  提取到 {len(cards)} 条笔记")
...
async def run_xhs_scraper():
    """运行小红书采集主流程"""
    all_results = []
    keywords = SEARCH_KEYWORDS.get("xiaohongshu", [])

    for keyword in keywords:
        eggs = await scrape_xiaohongshu(keyword)
    ...
    output_path = os.path.join(OUTPUT_DIR, f"xhs_eggs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump([e.to_dict() for e in unique_results], f, ensure_ascii=False, indent=2)

    print(f"\n✅ 小红书采集完成: {len(unique_results)} 条数据 → {output_path}")
